...
sumSquares a b = sumInts (sq a) (sq b)
...
higherOrderSum intApplication a b
    | a < b = intApplication a + higherOrderSum intApplication (succ a) b
    | otherwise = 0
...
reverse2 :: [a] -> [a]
reverse2 = foldr (\x acc -> x:acc) [] 

filter' :: [a] -> [a]  
filter' = foldr (\x acc -> acc) []
...
higherOrderSequenceApplication f op zero a b
  | a > b = zero
  | otherwise = op (f a) (higherOrderSequenceApplication f op zero (a + 1) b)
